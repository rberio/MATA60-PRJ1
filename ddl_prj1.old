-- =============================================================
-- PROJETO PRJ1 - MATA60 (Marco 1)
-- DDL 100% SQL (ANSI), pronto para PostgreSQL 12+
-- =============================================================

-- (Opcional) Schema dedicado ao projeto
-- CREATE SCHEMA prj1 AUTHORIZATION CURRENT_USER;
-- SET search_path TO prj1, public;
...
CREATE TABLE TB_FEEDBACK (
    ...
    CONSTRAINT FK_TB_FEEDBACK_PARTICIPANTE
        FOREIGN KEY (ID_PARTICIPANTE)
        REFERENCES TB_PARTICIPANTE (ID_PARTICIPANTE)
        ON UPDATE CASCADE
        ON DELETE CASCADE,
    CONSTRAINT FK_TB_FEEDBACK_ATIVIDADE
        FOREIGN KEY (ID_ATIVIDADE)
        REFERENCES TB_ATIVIDADE (ID_ATIVIDADE)
        ON UPDATE CASCADE
        ON DELETE CASCADE
...
CREATE TABLE TB_CERTIFICADO (
    ...
    CONSTRAINT FK_TB_CERTIF_PARTICIPANTE
        FOREIGN KEY (ID_PARTICIPANTE)
        REFERENCES TB_PARTICIPANTE (ID_PARTICIPANTE)
        ON UPDATE CASCADE
        ON DELETE CASCADE,
    CONSTRAINT FK_TB_CERTIF_ATIVIDADE
        FOREIGN KEY (ID_ATIVIDADE)
        REFERENCES TB_ATIVIDADE (ID_ATIVIDADE)
        ON UPDATE CASCADE
        ON DELETE CASCADE
...
-- Se desejar comentar o banco (substitua NOME_DO_BANCO por seu DB real):
-- COMMENT ON DATABASE NOME_DO_BANCO IS 'PRJ1 - Banco do Sistema de Gestão de Atividades de Extensão Universitária';
