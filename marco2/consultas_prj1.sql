...
/* REQ-13 - VERSÃO AVANÇADA
   Título: Elegíveis a certificado (confirmados + feedback) com análise de ranking
   Justificativa: Elegibilidade de emissão com análise comparativa por participante */
WITH elegiveis AS (
  SELECT
    p.id_participante,
    p.nm_participante,
    COUNT(DISTINCT i.id_atividade) AS atividades_elegiveis,
    AVG(f.vl_nota_conteudo + f.vl_nota_instrutor + f.vl_nota_organizacao) / 3.0 AS media_geral
  FROM tb_participante p
  JOIN tb_inscricao i ON i.id_participante = p.id_participante 
    AND i.st_confirmada = TRUE
  JOIN tb_feedback f ON f.id_participante = p.id_participante
...
ranking AS (
  SELECT
    ...
    media_geral,
    RANK() OVER (ORDER BY atividades_elegiveis DESC, media_geral DESC) AS posicao_ranking,
    PERCENT_RANK() OVER (ORDER BY atividades_elegiveis DESC) AS percentil,
    LAG(atividades_elegiveis) OVER (ORDER BY atividades_elegiveis DESC) AS atividades_anterior
  FROM elegiveis
)
SELECT
  id_participante,
  ...
  posicao_ranking,
  ROUND(percentil::numeric, 3) AS percentil,
  CASE 
    WHEN atividades_anterior IS NULL THEN 'Líder'
    WHEN atividades_elegiveis = atividades_anterior THEN 'Empatado'
    ELSE 'Abaixo'
  END AS status_comparativo
FROM ranking
ORDER BY posicao_ranking, id_participante
FETCH FIRST 100 ROWS ONLY;
